...
Fifty-three decisions numbered to 0054 — **0042 was never used**, and the gap is left
...
| 0027 | Import from YouTube via bundled yt-dlp; retires ADR-0014's zero-network invariant. | **Proposed — deferred, post-MVP** | 08-12 | Wireframe review |
...
| 0014 | Zero network requests, any code path. | `fetch` / `https` / `electron-updater` / a release-check on launch — and ARCHITECTURE §8.4's security-chapter cut is retroactively invalid. |
...
| **Import from YouTube** | **Open — deferred 08-12, out of MVP.** Argument drafted in ADR-0027 and not taken: yt-dlp fetches `bestaudio`, the file becomes `sourcePath` in a new `userData/sources/`, the item lands `imported`, and the existing pipeline does the rest. The blocker is not the feature — it is that the first network call retires ADR-0014's invariant and **owes ARCHITECTURE §8.4 the security chapter it cut**. Second cost, unsolved: yt-dlp's extractors break on YouTube's schedule and ADR-0014 ships no updater, so a bundled copy rots between releases (ADR-0027 option D — the user supplies the binary — is the live fallback). | ADR-0027, ADR-0014, ARCHITECTURE §8.4 |
